...
class QuantumWire:
    ...
    def get_ipr_and_shannon_entropy(self, window_width=50):
        """
        ...
        for psi in range(self.n-window_width,self.n+window_width):
            pm_psi = np.dot(np.conjugate(self.eigvecs[self.num_mol:,psi]),self.eigvecs[self.num_mol:,psi])
            for i in range(self.num_mol,self.num_mol*2):
                p = np.conjugate(self.eigvecs[i,psi])*self.eigvecs[i,psi]/pm_psi
                pi += p**2
                p = p.real
                s  -= p*np.log(p)
    ...
    def get_r_index_everyother(self, window_width=50):
        """
        Gets the r index as defined in eqn. 10 of 10.1016/j.aop.2021.168469.
        0.5307 is the theoretical GOE result, and Poisson is 0.3863.
        """
...
def get_properties(qw, nreal, fname=None, window_width=50):
    ...
    for r in range(nreal):
        ...
        w[r,1:] = qw.get_ipr_and_shannon_entropy(window_width)
